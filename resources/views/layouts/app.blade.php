...
<head>
    ...
    <!-- Custom CSS -->
    <link rel="stylesheet"
        href="{{ asset('css/style.css') }}?v={{ filemtime(public_path('css/style.css')) }}&fix=20260801">
    <!-- TomSelect CSS & JS -->
        ...
        <div id="page-content-wrapper">
            <header class="app-topbar">
                <div class="d-flex align-items-center gap-3">
                    <button class="btn btn-white d-lg-none" type="button" data-sidebar-toggle
                ...
                <div class="d-flex align-items-center gap-3 flex-grow-1 justify-content-end">
                    <form class="topbar-search" method="GET" action="{{ route('clients.index') }}" role="search">
                        ...
                        <input type="search" name="search" value="{{ request('search') }}" class="form-control"
                            placeholder="Search clients, invoices, services" aria-label="Search">
                    </form>
                    @auth('staff')
                        @php
                        ...
                        @endphp
                        <div class="dropdown">
                            <button class="btn btn-white d-flex align-items-center gap-2" data-bs-toggle="dropdown"
                                type="button" aria-expanded="false">
                                <span class="avatar-initials"
                                    style="width:30px;height:30px;font-size:.75rem">{{ substr($staffUser->name, 0, 2) }}</span>
                            ...
                            <div class="dropdown-menu dropdown-menu-end shadow border-0 rounded-3 p-0"
                                ...
                                <div class="p-3 border-bottom bg-light rounded-top-3">
                                    <div class="d-flex align-items-center gap-2 mb-1">
                                        <span class="avatar-initials"
                                            style="width:36px;height:36px;font-size:.85rem">{{ strtoupper(substr($staffUser->name, 0, 2)) }}</span>
                                        <div class="min-w-0 flex-grow-1">
                                            <div class="fw-semibold text-dark text-truncate" title="{{ $staffUser->name }}">
                                                {{ $staffUser->name }}</div>
                                            <div class="text-muted small text-truncate" title="{{ $staffUser->email }}">
                                                {{ $staffUser->email }}</div>
                                        </div>
                                    </div>
                                </div>
